			...
			if ( WCREAPDF_Helper::check_if_needs_attachement( $order ) ) {
				...
				if ( isset( $status ) && in_array ( $status, $allowed_stati ) ) {
					...
					if ( $option_on_or_off == 'on' ) {
						...
						$attachments[ 'gm_retoure' ] = WCREAPDF_TEMP_DIR . 'pdf' .  DIRECTORY_SEPARATOR . $directory_name . DIRECTORY_SEPARATOR . get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_file_name' ), __( 'Retoure', 'woocommerce-german-market' ) ) . '.pdf';
